...
/**
 * This manager-class derives most of its methods to others to separate workload.
 */
public class GameManager implements Manager, ControllerListener, MoveListener {
    ...
    @Override
    public void move(BoardState boardState, Position current, Position destination) {
        ...
        thread = new Thread(runnable, "Thread 1");
        thread.start();
    }
...
//        String report = "Game ended:";
        if (boardState.getBoard().getBlackLeft() == 1)
    ...
    private void startAI(){
        ...
        logger.log(Logger.LogType.INFO, "Tree fill time: " + (endTime - startTime));

        startTime = System.currentTimeMillis();
        BoardState tree = algorithm.search(treeFiller.getCurrentNode(), true);
        endTime = System.currentTimeMillis();
        logger.log(Logger.LogType.INFO, "Minimax time: " + (endTime - startTime));
